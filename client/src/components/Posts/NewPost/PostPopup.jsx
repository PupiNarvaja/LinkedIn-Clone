...
import { postPost } from "../../../redux/features/postsSlice";
...
const PostPopup = ({ openPopup, closePostPopup }) => {
  ...
  const sendPost = async () => {
    const dataToSend = { content: post.trim() };
    const { data } = await usePostRequest("http://localhost:8080/api/posts", dataToSend);
  ...
  return (
    <div className="w-[100vw] h-[100vh] top-0 left-0 fixed z-[1]">
        ...
        {isLoading ? (
          ...
          <>
            <div className="px-6 py-3 flex items-end">
              <img src={user?.profile} alt={`${user?.firstname} ${user?.lastname}`} className="w-12 h-12 rounded-full object-cover" />
